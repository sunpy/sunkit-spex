...
Based on the `Astropy JointFitter example`_

.. [Astropy JointFitter example] https://docs.astropy.org/en/stable/modeling/jointfitter.html
...
ax.plot(x1, y1, 'bo', alpha=0.4)
ax.plot(x2, y2, 'go', alpha=0.4)
...
# get models
gjf1 = Gaussian1D(amplitude=5, mean=5.5, stddev=0.4, 
                  bounds={'mean': (0, 10),
                          },
                  fixed={"stddev": True,
                         })
...
fit_joint =  fitters.JointFitter(
   optimizer=SLSQP, statistic=leastsquare
)
...
# ``modelN``, ``xN``, ``yN`` then we can pass as many model-data groupd 
# as we want.
...
ax.plot(x1, y1, 'bo', alpha=0.4)
ax.plot(x2, y2, 'go', alpha=0.4)
ax.plot(x1, g12[0](x1), 'b--', label='AreaGaussian1')
ax.plot(x2, g12[1](x2), 'g--', label='AreaGaussian2')
ax.set(xlabel='Wavelength', ylabel='Flux')
plt.legend()
plt.show()
...
row_labels = (
    [f"gjf1 {g12pn}" for g12pn in g12[0].param_names] + [f"gjf2 {g12pn}" for g12pn in g12[1].param_names]
)
row_labels[1] += "\n(bounded)"
row_labels[2] += "\n(fixed)"
row_labels[4] += "\n(tied to `gjf1 mean`)"
