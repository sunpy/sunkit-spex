...
data_gjf1 = ThermalEmission(temperature=data_temp, 
...
data_gjf2 = ThermalEmission(temperature=data_temp, 
...
# get models
gjf1 = ThermalEmission(temperature=10<<u.MK, 
...
gjf2 = ThermalEmission(emission_measure=0.1<<u.cm**-3, 
                       fixed={"mg": True,
                              "al": True,
                              "si": True,
                              "s": True,
...
# really give the fitter a difficult time when optimising calues on 
...
ax.set(xlabel=f"Energy [{x1.unit:latex}]", ylabel=f"{y1.unit:latex}")
plt.ylim([np.min([np.min(y1.value), np.min(y2.value)]), 
            np.max([np.max(y1.value), np.max(y2.value)])])
plt.yscale("log")
plt.xscale("log")
plt.legend()
plt.show()
