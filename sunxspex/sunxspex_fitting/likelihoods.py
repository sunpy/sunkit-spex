...
class LogLikelihoods:
    ...
    def remove_non_numbers(self, _lhoods):
        ...
        '''
        _lhoods = self.remove_nans(_lhoods)
        return _lhoods[np.isfinite(_lhoods)]
    
    def check_numbers_left(self, remaining):
    ...
    def gaussian_loglikelihood(self, model_counts, observed_counts, observed_count_errors):
        ...
        return self.check_numbers_left(self.remove_non_numbers(likelihoods)) # =ln(L)
    ...
    def poisson_loglikelihood(self, model_counts, observed_counts, observed_count_errors):
        ...
        return self.check_numbers_left(self.remove_non_numbers(likelihoods))
    
    def cash_loglikelihood(self, model_counts, observed_counts, observed_count_errors):
        ''' Cash log-likelihood. A simplification of the poissonian log-likelihood where the 
        independent data term is neglected. Since the data term is neglected the absolute number 
        does not say much about the absolute goodness of fit as more observations means a higher 
        natural number.
    ...
    def cstat_loglikelihood(self, model_counts, observed_counts, observed_count_errors):
        ''' C-stat log-likelihood. A simplification of the poissonian log-likelihood where the 
        independent data term is replaced using sterling's approximation. Used in the XSPEC fitting 
        software. Unlike the cash statistic, the resulting value retains some meaning of the 
