...
def load_spec(spec_file: str, time_base: str) -> SpecFileInfo:
    ...
    with fits.open(spec_file) as sf:
        ...
        slice_unit = lambda hdu, c: sf[hdu].columns[c].unit

        time_mids = sf['data'].data['time'] << u.Unit(slice_unit('data', 'time'))
        dt = TimeDelta(sf['data'].data['timedel'] << u.Unit(slice_unit('data', 'timedel')))
        ...
        t_lo = time_mids - np.floor(dt / 2)
        t_hi = time_mids + np.ceil(dt / 2)

        start_date = Time(sf['primary']['header']['date-beg'], format='isot', scale='utc')
        if time_base == 'utc':
            solo_earth_light_timedelta = sf['primary']['ear_tdel'] << u.s
        ...
        channel_bins = np.column_stack((
            sf['energies'].data['e_low'],
            sf['energies'].data['e_high']
        ...
        trig_comp_err = sf['data'].data['trigger_comp_er']
        ...
        count_rate = counts / (livetime_dat['value'][:, None] * time_bins) / np.diff(channel_bins, axis=1)
        # count rate error is affected by counts error as well as the livetime error.
        # do it proportionally to make the math cleaner
        count_rate_error = count_rate * np.sqrt(
            (counts_error / counts)**2 +
            (livetime_dat['error'] / livetime_dat['value'])**2
        )
...
def load_srm(srm_file: str, srm_choice='unattenuated') -> SrmFileInfo:
    ...
    with fits.open(srm_file) as sf:
        ...
        srm = sf[srm_idx]['matrix'] << (u.ct / u.keV / u.ph)
        count_bins = np.column_stack(
            (sf['ebounds']['e_min'], sf['ebounds']['e_max'])
    ...
    return SrmFileInfo(
        ...
        srm=(srm * np.diff(count_bins, axis=1) * area).to_value(u.cm**2 * u.ct / u.ph)
    )
