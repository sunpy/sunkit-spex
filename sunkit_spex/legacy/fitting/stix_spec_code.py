...
def _get_spec_file_info(spec_file):
    ...
    time_diff_so2e = sdict[
        "0"
    ][
        0
    ][
        "EAR_TDEL"
    ]  # time difference between Sun2Earth and Sun2SO, so time at earth for measurement is this difference added on to the actual detection time
    ...
    spec_stimes = [
        Time(sdict["0"][0]["DATE-BEG"], format="isot", scale="utc")
        + TimeDelta(time_diff_so2e * u.s)
        + TimeDelta(dt * u.cs)
        for dt in t_lo
    ]
    spec_etimes = [
    ...
    lvt = np.ones(
        (len(t_lo), len(channel_bins))
    )  # livetimes, (rows,columns) -> (times, channels), for STIX it is 1 at the minute?
