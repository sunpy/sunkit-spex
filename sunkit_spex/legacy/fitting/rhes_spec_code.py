...
def _get_spec_file_info(spec_file):
    ...
    times_s = rdict[
        ...
        "TIME"
    ]  # times of spectra, entries -> times. Times from start of the day of "DATE_OBS"; e.g.,"DATE_OBS"='2002-10-05T10:38:00.000' then times measured from '2002-10-05T00:00:00.000'
    ...
    counts, counts_err, cts_rates, cts_rate_err = _spec_file_units_check(
        rhessi_dict=rdict, livetimes=lvt, time_dels=time_deltas, kev_binning=channel_bins
    )
...
def _spec_file_units_check(rhessi_dict, livetimes, time_dels, kev_binning):
    ...
    if rhessi_dict["1"][0]["TTYPE1"] == "RATE":
        ...
        # **** Not Tested ****
        _flux = rhessi_dict["1"][1][
            "FLUX"
        ]  # flux for every time, (rows,columns) -> (times, channels) [counts/sec/cm^2/keV]
        cts_rates = _flux * rhessi_dict["1"][0]["GEOAREA"] * np.diff(kev_binning, axis=1).flatten()
        cts_rate_err = (
            rhessi_dict["1"][1]["STAT_ERR"] * rhessi_dict["1"][0]["GEOAREA"] * np.diff(kev_binning, axis=1).flatten()
        )
        counts = cts_rates * livetimes * time_dels[:, None]
        counts_err = cts_rate_err * livetimes * time_dels[:, None]
    else:
...
def _get_srm_file_info(srm_file):
    ...
    photon_bins = np.concatenate(
        (np.array(photon_channels_elo)[:, None], np.array(photon_channels_ehi)[:, None]), axis=1
    )
