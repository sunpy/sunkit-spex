...
class StixLoader(InstrumentBlueprint):
    ...
    def __init__(self, pha_file, srm_file=None, srm_custom=None, custom_channel_bins=None, custom_photon_bins=None,
        ...
        self._construction_string = f"StixLoader(pha_file={pha_file},srm_file={srm_file},srm_custom={srm_custom},custom_channel_bins={custom_channel_bins},custom_photon_bins={custom_photon_bins},**{kwargs})"
    ...
    def _load1spec(self, f_pha, f_srm, srm=None, channel_bins=None, photon_bins=None):
        ...
        if type(srm) == type(None):
            ...
            data_inds2match = np.where((obs_channel_bins[0, 0] <= srm_channel_bins[:, 0]) & (
                        srm_channel_bins[:, 1] <= obs_channel_bins[-1, -1]))
        ...
        counts_err = np.sqrt(
            np.sum(self._data_time_select(stime=self._full_obs_time[0], full_data=self._counts_err_perspec,
                                          etime=self._full_obs_time[1]) ** 2, axis=0))

        count_rate_error = np.sqrt(
            np.sum(self._data_time_select(stime=self._full_obs_time[0], full_data=self._count_rate_error_perspec,
                                          etime=self._full_obs_time[1]) ** 2, axis=0))

        _livetimes = np.mean(self._data_time_select(stime=self._full_obs_time[0], full_data=self._lvt_perspec,
                                                    etime=self._full_obs_time[1]),
                             axis=0)  # to convert a model count rate to counts, so need mean
    ...
    def data2data_minus_background(self, boolean):
        ...
        if boolean:
            ...
            if not hasattr(self, "_full_data"):
                self._full_data = {"counts": self._loaded_spec_data["counts"],
            ...
            new_cts = self._full_data["counts"] - (
                        self._loaded_spec_data["extras"]["background_rate"] * self._loaded_spec_data[
                    "effective_exposure"] * self._loaded_spec_data["count_channel_binning"])
            new_cts_err = np.sqrt(
                self._full_data["count_error"] ** 2 + (self._loaded_spec_data["extras"]["background_count_error"] *
                                                       (self._loaded_spec_data["effective_exposure"] /
                                                        self._loaded_spec_data["extras"][
                                                            "background_effective_exposure"])) ** 2)
            new_cts_rates = self._full_data["count_rate"] - self._loaded_spec_data["extras"]["background_rate"]
            new_cts_rates_err = np.sqrt(self._full_data["count_rate_error"] ** 2 + self._loaded_spec_data["extras"][
                "background_rate_error"] ** 2)

            self._loaded_spec_data.update({"counts": new_cts, "count_error": new_cts_err, "count_rate": new_cts_rates,
    ...
    def _update_event_data_with_times(self):
        ...
        self._loaded_spec_data["counts"] = np.sum(
            self._data_time_select(stime=self._start_event_time, full_data=self._counts_perspec,
                                   etime=self._end_event_time), axis=0)

        counts_err = np.sqrt(np.sum(
            self._data_time_select(stime=self._start_event_time, full_data=self._counts_err_perspec,
                                   etime=self._end_event_time) ** 2, axis=0))
        ...
        _livetimes = np.mean(self._data_time_select(stime=self._start_event_time, full_data=self._lvt_perspec,
                             ...
                             axis=0)  # to convert a model count rate to counts, so need mean
        _actual_first_bin = \
        self._data_time_select(stime=self._start_event_time, full_data=self._time_bins_perspec[:, 0],
                               etime=self._end_event_time)[0]
        _actual_last_bin = self._data_time_select(stime=self._start_event_time, full_data=self._time_bins_perspec[:, 1],
                                                  etime=self._end_event_time)[-1]
        self._loaded_spec_data["effective_exposure"] = np.diff([_actual_first_bin, _actual_last_bin])[0].to_value(
            "s") * _livetimes
        ...
        self._loaded_spec_data["count_rate"] = np.sum(
            self._data_time_select(stime=self._start_event_time, full_data=self._count_rate_perspec,
                                   etime=self._end_event_time), axis=0) / self._loaded_spec_data[
                                                   "count_channel_binning"]
        self._loaded_spec_data["count_rate_error"] = np.sqrt(np.sum(
            self._data_time_select(stime=self._start_event_time, full_data=self._count_rate_error_perspec,
                                   etime=self._end_event_time) ** 2, axis=0)) / self._loaded_spec_data[
                                                         "count_channel_binning"]
    ...
    def _update_bg_data_with_times(self):
        ...
        if (type(self._start_background_time) != type(None)) and (type(self._end_background_time) != type(None)):
            ...
            _livetimes = np.mean(self._data_time_select(stime=self._start_background_time, full_data=self._lvt_perspec,
                                 ...
                                 axis=0)  # to convert a model count rate to counts, so need mean
            _actual_first_bin = \
            self._data_time_select(stime=self._start_background_time, full_data=self._time_bins_perspec[:, 0],
                                   etime=self._end_background_time)[0]
            _actual_last_bin = \
            self._data_time_select(stime=self._start_background_time, full_data=self._time_bins_perspec[:, 1],
                                   etime=self._end_background_time)[-1]
            ...
            self._loaded_spec_data["extras"]["background_rate"] = self._loaded_spec_data["extras"][
                                                                      "background_counts"] / \
                                                                  self._loaded_spec_data["extras"][
                                                                      "background_effective_exposure"] / \
                                                                  self._loaded_spec_data["count_channel_binning"]
            self._loaded_spec_data["extras"]["background_rate_error"] = np.sqrt(
                self._loaded_spec_data["extras"]["background_counts"]) / \
                                                                        self._loaded_spec_data["extras"][
                                                                            "background_effective_exposure"] / \
                                                                        self._loaded_spec_data["count_channel_binning"]
    ...
    def start_background_time(self, bg_stime):
        ...
        _set_bg_stime = (not hasattr(self, "_end_background_time")) or (type(_t) == type(None)) or (
                hasattr(self, "_end_background_time") and (
                    (type(self._end_background_time) == type(None)) or (_t < self._end_background_time)))
        if _set_bg_stime:
    ...
    def end_background_time(self, bg_etime):
        ...
        _set_bg_etime = (not hasattr(self, "_start_background_time")) or (type(_t) == type(None)) or (
    ...
    def lightcurve(self, energy_ranges=None, axes=None, rebin_time=1):
        ...
        if type(energy_ranges) == type(None):
            energy_ranges = [[self._loaded_spec_data["count_channel_bins"][0, 0],
        ...
        for er in energy_ranges:
            i = np.where((self._loaded_spec_data["count_channel_bins"][:, 0] >= er[0]) & (
        ...
        _y_pos = ax.get_ylim()[0] + (
                    ax.get_ylim()[1] - ax.get_ylim()[0]) * 0.95  # stop region label overlapping axis spine
        if hasattr(self, "_start_background_time") and (type(self._start_background_time) != type(None)) and hasattr(
                self, "_end_background_time") and (type(self._end_background_time) != type(None)):
            ax.axvspan(*self._atimes2mdates([self._start_background_time, self._end_background_time]), alpha=0.1,
                       color='orange')
            ax.annotate("BG", (self._atimes2mdates([self._start_background_time])[0], _y_pos), color='orange', va="top",
                        size=_def_fs - 2)
        ...
        if hasattr(self, "_start_event_time") and hasattr(self, "_end_event_time"):
            ...
            ax.annotate("Evt", (self._atimes2mdates([self._start_event_time])[0], _y_pos), color='purple', va="top",
                        size=_def_fs - 2)

        self._lightcurve_data = {"mdtimes": _ts, "lightcurves": _lcs, "lightcurve_error": _lcs_err,
                                 "energy_ranges": energy_ranges}
    ...
    def spectrogram(self, axes=None, rebin_time=1, rebin_energy=1, **kwargs):
        ...
        # plot event time range
        if hasattr(self, "_start_event_time") and hasattr(self, "_end_event_time"):
            ax.hlines(y=etop, xmin=start_evt_time, xmax=end_evt_time, alpha=0.9, color='#F37AFF', capstyle='butt',
                      lw=10)
