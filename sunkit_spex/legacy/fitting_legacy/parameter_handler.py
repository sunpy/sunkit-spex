...
class Parameters:
    ...
    def __init__(self, parameter_names, rparams=False):
        ...
        if not rparams:
            ...
            tied_stats = ["tie_" + s1 for s1 in parameter_names[:spec1_param_num]] * int(
                (len(parameter_names) / spec1_param_num) - 1
            )
    ...
    def _check_valid_table(self):
        ...
        for i, s in zip(list(self.param_status.index), list(self.param_status)):
            if (s not in ("free", "frozen")) and (not str(s).startswith("tie")):
                print(
                    ...
                    " for entry [Status, "
                    + str(i)
                    + "]. Must be 'free', 'frozen', or 'tie_paramName' or accepted synonym. Changing back.",
                )
        ...
        for i, b in zip(list(self.param_bounds.index), list(self.param_bounds)):
            try:
                ...
                length = 0  # this is because something that doesn't have __len__() method could be here which would cause a TypeError
            if (type(b) is not tuple) or (length != 2):
                print(
                    ...
                    " for entry [Bounds, "
                    + str(i)
                    + "]. Must be type tuple of length 2, i.e., (min,max) with min<max. Changing back.",
                )
    ...
    def _set_to_another_entry(self, item, new_value):
        ...
        new_value : pandas.core.series.Series
                Parameter table entry that the item parameter is to be tied to.
        ...
        if (new_value.name in self.param_name) and (item != new_value.name):
            ...
            print(
                ...
                "to itself (",
                new_value.name,
                ") would be equivalent to fixing this parameter. Nothing is being changed.",
            )
        else:
            print(
                "I don't know what you've set this to but nothing is being changed, sorry (", item, "=", new_value, ")."
            )
    ...
    def _change_state(self, item, new_value):
        ...
        if type(new_value) is dict:
            ...
            assert len(self.param_names) == len(new_value), (
                "List for column must be same length as parameters (" + str(len(self.param_names)) + ")."
            )
        ...
        if type(new_value) is dict:
            ...
            for val in new_value:
                ...
                if type(key) is str:
                    ...
                    print(
                        f"The value given ({val}) is not a valid parameter table input. Must be a string, int/float, or tuple length==2."
                    )
    ...
    def _str_num_or_tuple(self, val):
        ...
        key = None
        val, key = (
            (self._frozen_free_or_tie(value=val), "Status") if (type(val) is str) else (val, key)
        )  # is value given a string for the Status?
        val, key = (
            (val, "Value") if isnumber(val) else (val, key)
        )  # (type(val) in (int, float)) else (val, key) # is value given a int/float for the Value?
        val, key = (
            (val, "Bounds") if (type(val) is tuple) else (val, key)
        )  # is value given a tuple (length==2) for the Bounds?
        return val, key
    ...
    def _change_specfic_entry(self, item, new_value):
        ...
        if (item[0] in self.param_names + self.states) and (item[1] in self.param_names + self.states):
            ...
            try:
                self.parameter_info.at[index, col[0]] = (
                    new_value  # try to update the specific entry with .at[param_name,state]
                )
            except KeyError:
    ...
    def __setitem__(self, item, new_value):
        ...
        # index a specific entry in the table, e.g., self.parameter_info["param1", "Status"]="free" or elf.parameter_info["Status", "param1"]="free"
            ...
            print(
                "Invalid index ",
                item,
                ". Valid rows and column names are: ",
                self.param_names,
                " and ",
                self.states,
                ", respectively.",
            ...
            print(
                "Or invalid value ",
                new_value,
                ". You have not set the parameter to a list, dict, string, integer, float, tuple (len==2), or row of itself.",
            )
    ...
    def __getitem__(self, item):
        ...
        elif len(set(item)) == 2 and (
            (item[0] in self.param_names + self.states) and (item[1] in self.param_names + self.states)
        ):
            index = item[0] if (item[0] in self.param_names) else item[1]
            col = list(item)
            col.remove(index)
