...
# Shared parameter definitions for ThermalEmission, ContinuumEmission and LineEmission
_THERMAL_PARAMETERS = {
...
# Declaration order also doubles as the positional-argument order accepted by __init__
_THERMAL_PARAM_ORDER = tuple(_THERMAL_PARAMETERS)
_THERMAL_INIT_ORDER = (*_THERMAL_PARAM_ORDER, "abundance_type")


def _resolve_thermal_kwargs(cls, args, kwargs):
    """
    Merge positional/keyword thermal parameters against class defaults, in declaration order.
    """
    merged = dict(zip(_THERMAL_INIT_ORDER, args))
    merged.update(kwargs)
    abundance_type = merged.pop("abundance_type", DEFAULT_ABUNDANCE_TYPE)
    for pname in _THERMAL_PARAM_ORDER:
        param = getattr(cls, pname)
        merged.setdefault(pname, u.Quantity(param.default, param.unit) if param.unit is not None else param.default)
    return merged, abundance_type


def _thermal_signature(cls):
    """
    Build an  __init__ signature (for docs/help/etc) matching the
    positional/keyword parameters __init__ actually accepts via _resolve_thermal_kwargs,
    since the real signature is the generic (*args, **kwargs).
    """
    params = [inspect.Parameter("self", inspect.Parameter.POSITIONAL_OR_KEYWORD)]
    for pname in _THERMAL_PARAM_ORDER:
        param = getattr(cls, pname)
        default = u.Quantity(param.default, param.unit) if param.unit is not None else param.default
        params.append(inspect.Parameter(pname, inspect.Parameter.POSITIONAL_OR_KEYWORD, default=default))
    params.append(
        inspect.Parameter("abundance_type", inspect.Parameter.POSITIONAL_OR_KEYWORD, default=DEFAULT_ABUNDANCE_TYPE)
    )
...
class ThermalEmission(FittableModel):
    ...
    # So don't have to copy the same parameters in each
    locals().update(_THERMAL_PARAMETERS)

    # input_units_equivalencies = {"keV": u.spectral(), "K": u.temperature_energy()}
    _input_units_allow_dimensionless = True

    def __init__(self, *args, **kwargs):
        kwargs, abundance_type = _resolve_thermal_kwargs(type(self), args, kwargs)
        self.abundance_type = abundance_type

        if abundance_type != DEFAULT_ABUNDANCE_TYPE:
            kwargs["mg"], kwargs["al"], kwargs["si"], kwargs["s"], kwargs["ar"], kwargs["ca"], kwargs["fe"] = (
                _initialize_abundances(DEFAULT_ABUNDANCES[abundance_type])
            )

        self.line = LineEmission(abundance_type=abundance_type, **kwargs)
        self.cont = ContinuumEmission(abundance_type=abundance_type, **kwargs)
...
# Restore the full signature for sphinx etc
...
class ContinuumEmission(FittableModel):
    ...
    .. plot::
        ...
        from sunkit_spex.models.physical.thermal import ContinuumEmission
...
class LineEmission(FittableModel):
    ...
    emission_measure = Parameter(
        name="emission_measure",
        default=1e50,  # why is this 1e50 when the rest were/are 1
        unit=(u.cm ** (-3)),
        description="Emission measure of the observer",
        fixed=False,
    )
...
# Restore the full signature for sphinx etc
LineEmission.__init__.__signature__ = _thermal_signature(LineEmission)
...
def _initialize_abundances(abundances: Column) -> tuple[float, ...]:
    ...
    mg = float(12 + np.log10(abundances[11]))
    al = float(12 + np.log10(abundances[12]))
    si = float(12 + np.log10(abundances[13]))
    s = float(12 + np.log10(abundances[15]))
    ar = float(12 + np.log10(abundances[17]))
    ca = float(12 + np.log10(abundances[19]))
    fe = float(12 + np.log10(abundances[25]))
