...
def check_metric_inputs(func):
  ...
  def wrapper(data_ys, model_ys, **kwargs):
    ...
    if len(data_ys)!=len(model_ys):
       raise ValueError(f"Inputs `data_ys_tuple` and `model_ys_tuple` must be tuples of of same length.")

    if "data_y_weights" in kwargs:
        if (kwargs["data_y_weights"] is not None) and (len(data_ys)!=len(kwargs["data_y_weights"])):
                raise ValueError(f"Inputs `data_ys_tuple` and `data_y_weights` must be tuples of of same length if `data_y_weights` is not `None`.")
